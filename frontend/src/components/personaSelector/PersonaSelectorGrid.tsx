      ...
      {personas.map((persona) => (
        <Grid item xs={12} sm={4} md={4} lg={4} xl={4} key={persona.id} sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
          <IconButton onClick={() => onSelect(persona)} sx={{ p: 0, mb: 1.5, '&:hover': { background: 'transparent' } }}>
            <Avatar src={persona.avatar} sx={{ width: 104, height: 104, mb: 0 }} />
          </IconButton>
          <Typography onClick={() => onSelect(persona)} sx={{ fontWeight: 700, fontSize: 20, color: '#222', textAlign: 'center', mb: 0.5, cursor: 'pointer', '&:hover': { textDecoration: 'underline' } }}>{persona.name}</Typography>
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, justifyContent: 'center' }}>
            <Typography onClick={() => onViewPersona(persona)} sx={{ color: '#219653', fontWeight: 500, fontSize: 16, textAlign: 'center', textTransform: 'capitalize', cursor: 'pointer', '&:hover': { textDecoration: 'underline' } }}>{persona.role}</Typography>
            <ArrowOutwardIcon sx={{ color: '#219653', fontSize: 18, mb: '-2px', cursor: 'pointer' }} onClick={() => onViewPersona(persona)} />
          </Box>
        </Grid>
      ))}
    </Grid>
