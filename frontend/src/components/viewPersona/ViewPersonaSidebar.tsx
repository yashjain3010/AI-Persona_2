    ...
    <List>
      {personas.map((p) => (
        ...
        >
          ...
          <ListItemText
            primary={<Typography sx={{ fontWeight: 700, fontSize: 16, color: '#222' }}>{p.name}</Typography>}
            secondary={<Typography sx={{ color: '#219653', fontWeight: 500, fontSize: 14 }}>{p.role}</Typography>}
          />
        </ListItem>
      ))}
    </List>
